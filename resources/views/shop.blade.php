                ...
                <ul class="list list-inline mb-0">
                  ...
                  <li class="list-item">
                    <span class="menu-link py-1">
                      ...
                      @if(strpos($f_categories, (string)$category->id) !== false) checked @endif />
                      {{$category->name}}
                    </span>
          ...
          <div class="accordion-item mb-4">
            ...
            <div id="accordion-filter-price" class="accordion-collapse collapse show border-0"
              ...
              <input class="price-range-slider" type="text" name="price_range" value="" data-slider-min="1"
                data-slider-max="500" data-slider-step="1" data-slider-value="[{{$min_price}},{{$max_price}}]" data-currency="$" />
            ...
            <div class="product-card mb-3 mb-md-4 mb-xxl-5">
              <div class="pc__img-wrapper">
                <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                  <div class="swiper-wrapper">
                    <div class="swiper-slide">
                      <a href="{{route('shop.details', ['product_slug' => $product->slug])}}"><img loading="lazy" src="{{asset('upload/products/'.$product->image)}}" width="330"
                          height="400" alt="{{$product->name}}" class="pc__img"></a>
                    </div>
                    <div class="swiper-slide">
                        @foreach(explode(',',$product->images) as $gimg)
                         <a href="{{route('shop.details', ['product_slug' => $product->slug])}}"><img loading="lazy" src="{{asset('upload/products/'.$gimg) }}" width="330"
                            height="400" alt="{{$product->name}}" class="pc__img"></a>
                        @endforeach
                    </div>
                  </div>
              ...
              <div class="pc__info position-relative">
                ...
                <h6 class="pc__title"><a href="{{route('shop.details', ['product_slug' => $product->slug])}}">{{$product->name}}</a></h6>
                ...
                <div class="product-card__review d-flex align-items-center">
                  ...
                  <span class="reviews-note text-lowercase text-secondary ms-1">8k+ reviews</span>
                      ...
                      @if (Cart::instance('wishlist')->content()->where('id',$product->id)->count()>0)
                       <form action="{{route('wishlist.remove_item',['rowId'=>Cart::instance('wishlist')->content()->where('id',$product->id)->first()->rowId])}}" method="post"> 
                      @csrf
                      @method('DELETE')
...
<form id="frmfilter" method="GET" action="{{route('shop.index')}}">
  ...
  <input type="hidden" name="brands" id="hbrands"  />
  <input type="hidden" name="categories" id="hcategories"  />
...
@push('scripts')
 ...
 $(function() {
    ...
    $('#orderby').on('change', function() {
        ...
        $('#frmfilter').submit();
    }); // <--- Đóng ngoặc sự kiện orderby ở đây

    // Sự kiện thay đổi Brand (Đưa ra ngoài)
    $("input[class='chk-brand']").on("change", function() {
        var brands = '';
        $("input[class='chk-brand']:checked").each(function() {
            if (brands == "") {
                brands += $(this).val();
            } else {
                brands += ',' + $(this).val();
    ...
    // Sự kiện thay đổi Category (Đưa ra ngoài)
     $("input[class='chk-category']").on("change", function() {
        var categories = '';
        $("input[class='chk-category']:checked").each(function() {
            if (categories == "") {
                categories += $(this).val();
            } else {
                categories += ',' + $(this).val();
    ...
    // Sự kiện thay đổi Price Range
    $("input[name='price_range']").on("slideStop",function(){
